...
class GameFragment : Fragment(), SensorEventListener, View.OnTouchListener{
    ...
    var mRotMatrix = floatArrayOf(  1f,0f,0f,
        0f,0.5f,-0.5f,
        0f,0.5f,0.5f)
    ...
    inner class GameView(context: Context?) : View(context) {
        ...
        override fun onDraw(canvas: Canvas) {
            ...
            if(orientation[1]<-0.1f) tilt = -0.1f
            else if(orientation[1]>0.1f) tilt = -0.1f
            else tilt = orientation[1]

            with(canvas){
                drawText(""+orientation[0], 10f, 300f,textPaint)
                drawText(""+orientation[1], 10f, 350f,textPaint)
                drawText(""+orientation[2], 10f, 400f,textPaint)
                drawText(""+tilt, 10f, 450f,textPaint)
